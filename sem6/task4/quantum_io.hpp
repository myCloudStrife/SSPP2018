#ifndef QUANTUM_IO_HPP_
#define QUANTUM_IO_HPP_
...
std::complex<double> * rand_vec_norm(unsigned long long n) {
    static int rank = -1;
    if (rank == -1) {
...
#pragma omp for reduction(+: sum)
        for (unsigned long long i = 0; i < n; ++i) {
            vec[i] = std::complex<double>(1.0 / rand_r(&seed), 1.0 / rand_r(&seed));
            sum += norm(vec[i]);
        }
    }
...
std::complex<double> * read_vec(const char *filename, unsigned long long & n, int size, int rank) {
    MPI_File file;
    ...
    std::complex<double> *vec = new std::complex<double>[n];
    MPI_File_seek(file, rank * n * sizeof(vec[0]) + sizeof(vec_fullsize), MPI_SEEK_SET);
...
void print_vec(std::complex<double> *vec, unsigned long long n, int size, int rank) {
    char ready = 0;
    if (rank) {
        MPI_Recv(&ready, 1, MPI_CHAR, rank - 1, MESSAGETAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    for (unsigned long long i = 0; i < n; ++i) {
